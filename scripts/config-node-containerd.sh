...
# Kubernetes Node节点配置脚本 - containerd版本
# 使用containerd作为容器运行时，配置Node节点环境
# 日期: $(date +%Y-%m-%d)
...
validate_config() {
    ...
    # 检查必需的配置项
    local required_vars=("KUBERNETES_VERSION" "DOCKER_VERSION")
...
read_config() {
    ...
    DOCKER_VERSION=$(grep "^DOCKER_VERSION=" "$config_file" | cut -d'=' -f2 | tr -d ' ' || echo "24.0.7")
...
# 显示加入命令提示
show_join_info() {
    log_info "Node节点环境配置完成！"
    echo "*********************************************************************************************************"
    echo "*   Node节点环境配置完成！                                                                              *"
    echo "*   使用containerd作为容器运行时                                                                         *"
    echo "*                                                                                                       *"
    echo "*   现在您可以使用以下命令加入Kubernetes集群：                                                          *"
    echo "*                                                                                                       *"
    ...
    echo "*                                                                                                       *"
    echo "*   请从Master节点获取正确的token和hash值                                                               *"
    echo "*********************************************************************************************************"
}
...
main() {
    ...
    log_info "开始Kubernetes Node节点环境配置（使用containerd）..."
    ...
    log_success "Kubernetes Node节点环境配置完成！使用containerd作为容器运行时"
}
